...
- **Not headless.** Any `msteams:` deep-link open makes Teams activate its *own* window (Chromium pulls itself forward), so a send — or a `read` that navigates to a named chat — briefly brings Teams to the front. The binary then **restores focus** to the app you were using, via a settle-then-retry loop, so you end up back where you started.
